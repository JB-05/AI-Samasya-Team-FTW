...
-- - Core tables (observers, learners, sessions, pattern_snapshots, trend_summaries)
-- - Learner access codes (for child app session ingestion)
-- - Reports table (derived, language-only artifacts)
--
-- This schema is:
-- - Minimal
-- - Privacy-preserving
...
-- =============================================================================
-- Table 1: observers (Adult Accounts)
...
-- =============================================================================
-- Table 2: learners (Alias-Based, Non-Identifying)
-- Rules:
-- - Alias chosen by adult
-- - No age, gender, grade, or name
-- - One observer owns their learners
-- - learner_code: unique access code for child app (write-only access)
-- =============================================================================
...
-- Function to generate random, non-identifying codes
...
-- Backfill learner_code for any existing learners (if migration is run on existing DB)
DO $$
BEGIN
    UPDATE learners
    SET learner_code = generate_learner_code()
    WHERE learner_code IS NULL OR learner_code = '';
END $$;
...
CREATE INDEX IF NOT EXISTS idx_learners_observer_id ON learners(observer_id);
...
-- =============================================================================
-- Table 4: pattern_snapshots (Core Intelligence Storage)
...
-- Indexes for faster lookups
CREATE INDEX IF NOT EXISTS idx_pattern_snapshots_session_id ON pattern_snapshots(session_id);
CREATE INDEX IF NOT EXISTS idx_pattern_snapshots_learner_id ON pattern_snapshots(learner_id);
CREATE INDEX IF NOT EXISTS idx_pattern_snapshots_pattern_name ON pattern_snapshots(pattern_name);
CREATE INDEX IF NOT EXISTS idx_pattern_snapshots_created_at ON pattern_snapshots(created_at);


-- =============================================================================
-- Table 5: trend_summaries (Derived, Optional Cache)
-- Rules:
...
-- Indexes for faster lookups
CREATE INDEX IF NOT EXISTS idx_trend_summaries_learner_id ON trend_summaries(learner_id);
CREATE INDEX IF NOT EXISTS idx_trend_summaries_pattern_name ON trend_summaries(pattern_name);


-- =============================================================================
-- Table 6: reports (Derived, Language-Only Artifacts)
